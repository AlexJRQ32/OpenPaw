...
/**
 * RegistroMapaUbicacion — mapa Leaflet posicional (T37, decisión PO: mismo
 * criterio que ProfileMap T28 / InventarioMap T34 / TrasladoMap T35).
 *
 * El DTO backend (`CrearVeterinariaDto`) SÍ acepta Latitud/Longitud
...
export function RegistroMapaUbicacion({
  direccion = '',
  ariaLabel = 'Mapa referencial de la ubicación de la veterinaria',
}) {
  ...
  return (
    <div className="vreg-map">
      <div
        className="vreg-map__canvas"
        ref={containerRef}
        role="img"
        aria-label={`${ariaLabel}. Zona: ${zonaTexto}`}
      />
      <div className="vreg-map__overlay">
        <span className="vreg-map__chip">{zonaTexto}</span>
        <button
          type="button"
          className="vreg-map__locate"
          onClick={recentrar}
...
export default RegistroMapaUbicacion
