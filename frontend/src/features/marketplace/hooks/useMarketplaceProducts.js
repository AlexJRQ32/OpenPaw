...
export function useMarketplaceProducts() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError('')
      try {
        // Deuda #70: marketplace público usa /inventario/publico con DTO mínimo (sin auth)
        const [invRes, vetRes, srvRes] = await Promise.all([
          fetch(`${API_BASE_URL}/inventario/publico`),
  ...
  // Deuda #70: soporta DTO público plano (inventario/publico) y forma anidada legacy
  const products = inventory
    .filter((item) => {
      // DTO público plano: stock + nombre; legacy anidado: cantidad + producto
      if (item.stock !== undefined) return item.stock > 0
      return item.cantidad > 0 && item.producto?.activo !== false
    })
    .map((item) => {
      // DTO público plano (InventarioPublicoDto)
      if (item.producto == null && item.nombre != null) {
        return {
          id: item.productoId,
          inventarioId: item.inventarioId ?? item.id ?? 0,
          nombre: item.nombre,
          ...
          stockMaximo: null,
          almacenId: null,
          almacenNombre: item.almacenNombre || 'Almacen',
          veterinariaId: item.veterinariaId ?? null,
          veterinariaNombre: item.veterinariaNombre || null,
        }
      }
      // Forma anidada legacy (InventarioDto) — compat con tests
      return {
        id: item.producto.id,
